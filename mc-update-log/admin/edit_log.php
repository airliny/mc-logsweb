...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_GET['upload_image'])) {
    ...
    if (empty($title)) {
        ...
        $stmt = $db->prepare("UPDATE update_logs SET title=:title, content=:content, mc_version=:mc_version, category=:category, tags=:tags, author_display=:author_display, status=:status, updated_at=datetime('now','localtime') WHERE id=:id");
...
<head>
    ...
    <style>
        ...
        .btn-danger { background: #f44336; color: #fff; margin-left: auto; }
    ...
    <main class="admin-main">
        ...
        <form class="form-card" method="POST" id="logForm">
            <div class="form-group">
                <label>日志标题 *</label>
                <input type="text" name="title" required value="<?= h($log['title']) ?>">
            </div>
            
            <div class="form-row">
                <div class="form-group">
                    <label>Minecraft 版本</label>
                    <input type="text" name="mc_version" list="versionList" value="<?= h($log['mc_version']) ?>">
                ...
                <div class="form-group">
                    <label>分类</label>
                    <input type="text" name="category" list="categoryList" value="<?= h($log['category']) ?>">
                ...
                <div class="form-group">
                    <label>发布者名称</label>
                    <input type="text" name="author_display" placeholder="留空使用用户名" value="<?= h($log['author_display']) ?>">
                    <div class="hint">自定义本条日志的发布者名称</div>
                </div>
                <div class="form-group">
                    <label>标签（逗号分隔）</label>
                    <input type="text" name="tags" value="<?= h($log['tags']) ?>">
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <textarea name="content" id="content" required><?= h($log['content']) ?></textarea>
            </div>
            
            <div class="form-actions">
                <button type="submit" name="status" value="published" class="btn btn-primary">📢 更新并发布</button>
                <button type="submit" name="status" value="draft" class="btn btn-secondary">💾 保存草稿</button>
                <a href="logs.php" class="btn btn-secondary">返回</a>
                <a href="delete_log.php?id=<?= $id ?>" class="btn btn-danger" onclick="return confirm('确定删除此日志吗？此操作不可恢复！')">🗑 删除</a>
            </div>
        </form>
    </main>
</div>
...
document.getElementById('imageInput').addEventListener('change', function(e) {
    ...
    fetch('edit_log.php?id=<?= $id ?>&upload_image=1', {
        method: 'POST',
        body: formData
    })
    .then(r => r.json())
